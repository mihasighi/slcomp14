...
We provide here only an informal description of this fragment. 
The semantics of this fragment chosen in the competition is the \emph{precise} semantics.
...
Let $x,y,z$ denote program variables and $X,Y,Z$ logical variables. Every variable has a fixed type.
The program variable $\nil$ has a fixed meaning representing an undefined (not allocated) location.
The logic specifies (non-)aliasing relation between variables (logical or program) using (dis-)equality atoms, e.g., $x = y$ or $x\neq z$. A conjunction of such atoms is called a \emph{pure formula}. 

Let $f,n,p,s$ denote field names used in the definition of records.
The logic includes three kinds of atomic propositions describing the heap, called also \emph{spatial atoms}:
(i) the empty heap $\mathit{emp}$, 
(ii) a heap consisting of one allocated record cell, e.g., $x \points \{\ldots\}$,
and
(iii) an unbounded heap segment, e.g., $\ls(E,F)$, corresponding to a data structure whose shape is defined by a finite set of \emph{inductive definitions} like the ones provided in Table~\ref{tab:RD}. 
The spatial atoms are connected via a separating conjunction primitive $\posep$ to form a \emph{spatial formula}. 
A formula is a conjunction of a pure and a spatial formula.

For example, the following formula specifies a heap built from two cells of singly linked list
(pointed by $x$ resp. $y$) and a singly list segment starting from $z$ and ending in $\nil$:
\begin{align}
x \points \{(f,y)\} \ \posep\ y \points \{(f,z)\} \ \posep\ \ls(z,\nil)
\end{align}
The definition $\ls$ on Table~\ref{tab:RD} specifies acyclic singly linked list segments starting in $E$ and with last element pointing to $F$; the recursive field is $f$.
The list is acyclic because the non-aliasing pure constraint $E\neq F$ prevents $F$ to be one of the cells inside the list segment.

Table~\ref{tab:RD} provides other examples for inductive definitions.
%
The atom $\nll(E,F,B)$ specifies a list segment (of successor field $s$) starting from $E$ where each cell has a reference (in field $h$) to a list segment ($\ls$, of successor field $f$) ending to some common border location $B$. 
%
The doubly linked list segment $\dll(E,L,P,F)$ starts in $E$ and has as last element $L$; $P$ denotes the location stored by the ``previous'' field $p$ of $E$ and $F$ denotes the location stored in the ``next'' field $n$ of $L$.
%
The definition $\mathtt{btree}(E)$ specifies a binary tree with root $E$:
the first disjunct specifies the case of an empty tree, i.e., with $\nil$ root, while the second disjunct defines a tree with at least one cell allocated in $E$ separated from the trees rooted in its field for the left $l$ and right $r$ sons.
%
The last definition $\mathtt{tll}$ specifies a heap storing a binary tree rooted at $R$ and having at least one cell (required by the first disjunct),
where the leaves are linked using a doubly linked list (of fields $n$ and $p$)  between $P$ and $F$ (excluded).
%
The general syntax of inductive definitions is the following:
...
\\
&& \exists X,Y\sep E\points \{(r,X),(l,Y)\} \posep \mathtt{btree}(X) \posep \mathtt{btree}(Y)\big)
\nonumber 
\\[1mm]
%  
