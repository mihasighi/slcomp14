...
A SAT solver is called to check the satisfiability of the boolean abstraction and to infer the (dis)equalities implicit from the semantics of SL which are used to ``normalize'' the SL formula.
Then, a graph is built for each normalized formula and it is used to obtain a graph homomorphism that implies the entailment.
...
(ii) incorrect answers disqualify the solver.\fbox{to be checked!}
